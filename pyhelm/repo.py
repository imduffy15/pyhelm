...
class HTTPGetError(RuntimeError):
    def __init__(self, url, code, msg):
        super(RuntimeError, self).__init__("GET %s failed (%d): %s", url, code, msg)
...
class ChartError(RuntimeError):
    def __init__(self, chart=None):
        if chart:
            ...
            super(RuntimeError, self).__init__("%s not found in the repository" % chart)


class VersionError(RuntimeError):
    def __init__(self, version):
        super(RuntimeError, self).__init__("Chart version %s not found" % version)


class SchemeError(RuntimeError):
    def __init__(self, scheme):
        super(RuntimeError, self).__init__("The %s repository not supported" % scheme)


class RepositoryError(RuntimeError):
    def __init__(self, repository):
        super(RuntimeError, self).__init__("%s repository not found" % repository)


def _semver_sorter(x):
    return list(
        map(int, "".join(i for i in x["version"] if i in "0123456789.").split("."))
    )
...
def _get_from_s3(repo_url, file_url):
    ...
    try:
        file_object = s3_client.get_object(
            Bucket=file_url_parsed.netloc, Key=file_url_parsed.path.strip("/"),
        )
...
def repo_index(repo_url, headers=None):
    ...
    return yaml.load(
        _get_from_repo(repo_scheme, repo_url, "index.yaml", headers=headers,)
    )
