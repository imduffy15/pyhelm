...
class ChartBuilder(object):
    ...
    def source_clone(self):
        ...
        if "type" not in self.chart.source:
            self._logger.exception("Need source type for chart %s", self.chart.name)
            return
    ...
    def get_metadata(self):
        ...
        chart_yaml = yaml.load(
            ChartBuilder.read_file(os.path.join(self.source_directory, "Chart.yaml"))
        )
        ...
        for root, _, files in os.walk(self.source_directory):
            ...
            for file in files:
                ...
                filename = os.path.relpath(
                    os.path.join(root, file), self.source_directory
                )
    ...
    def get_values(self):
        ...
        if os.path.exists(os.path.join(self.source_directory, "values.yaml")):
            ...
            self._logger.warn(
                "No values.yaml in %s, using empty values", self.source_directory
            )
            raw_values = ""
    ...
    def get_templates(self):
        ...
        for root, _, files in os.walk(os.path.join(self.source_directory, "templates")):
            for tpl_file in files:
                template_name = os.path.relpath(
                    os.path.join(root, tpl_file), self.source_directory
                )
